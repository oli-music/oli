package com.oli.music.oliCanvas
...
@Serializable
data class oliCanvasManifest(
    val items: List<oliCanvasItem> = emptyList()
)

@Serializable
data class oliCanvasItem(
    val song: String,
    val artist: String,
    val url: String
)

object oliCanvasProvider {
    private const val BASE_URL = "https://canvas.oli.app/canvas.json"
    ...
    private suspend fun fetchManifest(): oliCanvasManifest? {
        ...
        return try {
            val manifest: oliCanvasManifest = client.get(BASE_URL).body()
