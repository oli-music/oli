package com.oli.music.appcore.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "echo_temp.zip") else File(downloadDir, "oli.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "oli.apk")
                var extracted = false
                try {
